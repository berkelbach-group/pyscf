...
def kernel(mp, mo_energy, mo_coeff, css=None, cos=None, kappa=None, verbose=logger.NOTE, with_t2=WITH_T2):
    nmo = mp.nmo
    nocc = mp.nocc
    nvir = nmo - nocc
    nkpts = mp.nkpts
    ...
    print("css", css, "cos", cos, "kappa", kappa) 
    ...
    for ki in range(nkpts):
      for kj in range(nkpts):
        ...
        for ka in range(nkpts):
            ...
            ## This part calculate the "coulomb" and "exchange" contribution separatly ###
            ...
            emp2p_reg += np.einsum('ijab,ijab', t2_ijab_reg, woovv).real
            emp2m_reg +=np.einsum('ijab,ijab', t2_ijab_reg, woovv_ex).real
    ...
    return emp2, emp2_reg, emp2_sos, emp2_scs, t2
...
class KMP2_SCS_reg(kmp2.KMP2):
    ...
    def __init__(self, mf, frozen=0, mo_coeff=None, mo_occ=None, css=1./3, cos=6./5, kappa=1.45):
...
# don't modify the following attributes, they are not input options
    ...
    def kernel(self, mo_energy=None, mo_coeff=None, css=None, cos=None,with_t2=WITH_T2):
        ...
        self.e_corr, self.emp2_reg, self.emp2_sos, self.emp2_scs, self.t2 = \
                kernel(self, mo_energy, mo_coeff, css, cos, verbose=self.verbose, with_t2=with_t2)
        logger.log(self, 'KMP2 energy             = %.15g', self.e_corr)
        logger.log(self, 'Regularized KMP2 energy = %.15g', self.emp2_reg)
        logger.log(self, 'SOS-KMP2 energy         = %.15g', self.emp2_sos)
        logger.log(self, 'SCS-KMP2 energy         = %.15g', self.emp2_scs)

        log.timer("KMP2", *cpu0)
        return self.e_corr, self.emp2_reg, self.emp2_sos, self.emp2_scs, self.t2
...
if __name__ == '__main__':
     ...
     mymp_scs=KMP2_SCS_reg(kmf)
     #mymp_scs.css=1
     #mymp_scs.cos=1
     emp2, kmp2_reg, emp2_sos, emp2_scs, t2=mymp_scs.kernel()
